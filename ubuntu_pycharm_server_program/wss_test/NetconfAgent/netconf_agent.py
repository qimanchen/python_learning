...
class NetconfAgent(object):
    ...
    def get_config_info(self):
        ...
        for port_state in all_connects:
            re_match = re.match(r'.*?<ingress>(\d+?)</ingress><egress>(\d+?)</egress>.*', port_state)
        ...
        for port_state in all_port_state:
            re_match = re.match(r'.*?<port-id>(\d+?)</port-id>.*?<port-state>(.*?)</port-state>.*', port_state)
    ...
    def delete_one_connect(self, ingress_port, egress_port):
        ...
        re_reply_info = re.match(r'.*?(ok).*', reply_info)
        if re_reply_info and re_reply_info.group(1) == 'ok':
            self.exist_connect.pop(str(ingress_port))
            logger.info("{0} with {1} connect is deleting successful", (ingress_port, egress_port))
...
if __name__ == '__main__':
    ...
    # new_session.modify_port_state(1, PORT_STATE[0])
    # time.sleep(5)
    # new_session.get_config_info()
    # new_session.get_all_connect()
